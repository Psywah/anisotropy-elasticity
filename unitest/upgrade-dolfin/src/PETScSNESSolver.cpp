                           ...
                           GenericVector& x)
{
  ...
  if (parameters["report"].is_set())
  {
    ...
    //SNESLineSearchMonitorSet(linesearch,SNESLineSearchMonitorDefault, NULL,NULL);
    else {
        SNESLineSearchMonitorCancel(linesearch);
    }
  }
...
void PETScSNESSolver::set_linear_solver_parameters()
{
  ...
  if (parameters("krylov_solver")["report"].is_set())
  {
    if (parameters("krylov_solver")["report"])
    {
  ...
  else if (PETScKrylovSolver::_methods.count(linear_solver) != 0)
  {
    ...
    {
        info("set additive_schwarz para");
        const int schwarz_overlap = krylov_parameters("preconditioner")("schwarz")["overlap"];
        PCASMSetOverlap(pc, schwarz_overlap);
        PCGASMSetType(pc, PC_GASM_BASIC); 
