  ...
  else if (std::string(parameters["nonlinear_solver"]) == "snes")
  {
    ...
    else
    {
       ...
       while(iter< max_iter)
       {
           ...
           {
               double value;
               res.vector()->get_local(&value, 1, &dof);
               if( value < infty*rho1)
               { 
                   nonlinear_coarse_problem->add_good_dof(dof, 0.0);
                   ++size_good;
               }
           }
...
NonlinearCoarseDiscreteProblem::F(GenericVector& b, const GenericVector& x)
{
  ...
  b.set_local(values.data(), values.size(), dofs.data());
  b.apply("insert");
                                                        ...
                                                        const GenericVector& x)
{
  ...
  A.ident_local(dofs.size(),dofs.data());
  A.apply("insert");
